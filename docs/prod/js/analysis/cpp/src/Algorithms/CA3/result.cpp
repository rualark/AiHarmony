...
uint8_t* buildResult(CGenCA3& gen) {
  if (result) {
    delete[] result;
    result = nullptr;
  }
  ...
  ui_b256(vec, gen.s_start * 2, 2);
  ui_b256(vec, gen.c_len * 2, 2);
  ui_b256(vec, gen.mode, 1);
  ui_b256(vec, gen.hli.size(), 2);
  for (int hs = 0; hs < gen.hli.size(); ++hs) {
    gen.hs = hs;
    ui_b256(vec, gen.hli[hs] * 2, 2);
    safeString_b256(vec, gen.GetChordName(), 1);
  }
